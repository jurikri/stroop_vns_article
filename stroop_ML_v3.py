...
for s in range(len(subject_list)):
    ...
    for i in vix2:
        ...
        tmpallo = np.array(X3_axis[0,:,fi,ch] )
        / np.sum(tmp, axis=2)[:, :, np.newaxis]
        
        X3_axis[0] = tmpallo
        
        Xml.append(X3_axis[0])
...
"""
X3 데이터 처리를 기존과 완전히 똑같게

"""

#%% intergrated score - Y

pload = r'C:\SynologyDrive\worik in progress\혁창패\임상실험 논문작성\analysis' + '\subject_tscore.pkl'
with open(pload, 'rb') as file:
    msdict = pickle.load(file)

total_keylist = msdict['total_keylist']
subject_x_se_x_tscore = msdict['subject_x_se_x_tscore']

ingroup = np.logical_or(Zml[:,-1]=='sham', Zml[:,-1]=='VNS')
Xml2, Yml2, Zml2 = [], [], []
# subject_list = list(set(Z_group[:,3]))
for s in range(len(total_keylist)):
    idix = Zml[:,3]==total_keylist[s]
    baseix = Zml[:,4]=='0'
    conditions = [ingroup, idix, baseix]
    vix = np.where(np.logical_and.reduce(conditions))[0]
    
    if len(vix) > 0:
        base = np.median(np.mean(np.median(Xml[vix], axis=1), axis=2), axis=0)
        
        idix = Zml[:,3]==total_keylist[s]
        baseix = Zml[:,4]=='1'
        conditions = [ingroup, idix, baseix]
        vix = np.where(np.logical_and.reduce(conditions))[0]  
        signal_se1 = np.mean(np.mean(np.median(Xml[vix], axis=1), axis=2), axis=0)
        
        idix = Zml[:,3]==total_keylist[s]
        baseix = Zml[:,4]=='2'
        conditions = [ingroup, idix, baseix]
        vix = np.where(np.logical_and.reduce(conditions))[0]  
        signal_se2 = np.mean(np.mean(np.median(Xml[vix], axis=1), axis=2), axis=0)
        
        # xtmp = (10*np.log10(signal_se1 / base))[4:40]
        xtmp = signal_se1
        Xml2.append(xtmp)
        Yml2.append(subject_x_se_x_tscore[s][1] - subject_x_se_x_tscore[s][0])
        Zml2.append(Zml[vix[0]][-1])
        
        # xtmp = (10*np.log10(signal_se2 / base))[4:40]
        xtmp = signal_se2
        Xml2.append(xtmp)
        Yml2.append(subject_x_se_x_tscore[s][2] - subject_x_se_x_tscore[s][0])
        Zml2.append(Zml[vix[0]][-1])
    else:
        print(total_keylist[s])

Xml2, Yml2 = np.array(Xml2), np.array(Yml2)
print(Xml2.shape, Yml2.shape)
...
Yml2, nmr_max, nmr_min = normalize(Yml2)

#%%
Zml2 = np.array(Zml2)
g0 = np.where(Zml2 == 'sham')
g1 = np.where(Zml2 == 'VNS')

plt.plot(np.mean(Xml2[g0], axis=0))
plt.plot(np.mean(Xml2[g1], axis=0))
#%%
...
model.fit(Xml2, Yml2, epochs=100, batch_size=10, verbose=1)

# 모델 평가 (필요 시 테스트 데이터로 평가)
loss = model.evaluate(Xml2, Yml2, verbose=0)
...
def keras_model(xinput):
    model = Sequential()
    model.add(Dense(units=32, activation='relu', input_shape=(xinput)))
...
spath = r'C:\\SynologyDrive\worik in progress\혁창패\임상실험 논문작성\analysis\MLmodel_figure4\\'
for i in tqdm(range(Xml2.shape[0])):
    model = keras_model(xinput=Xml2.shape[1:])
    
    tix = list(range(len(Xml2)))
    teix = set([i])
    trix = list(set(tix)-teix)
    teix = list(teix)
    
    X_train, _ = Xml2[trix], Xml2[teix]
    Y_train, _ = Yml2[trix], Yml2[teix]
    
    model.fit(X_train, Y_train, epochs=70, batch_size=2**6, verbose=0)
    model.save_weights(spath + f'model_fold_{i}.h5')
...
for i in tqdm(range(Xml2.shape[0])):
    model = keras_model(xinput=Xml2.shape[1:])
    model.load_weights(spath + f'model_fold_{i}.h5')
    ...
    _, X_test = Xml2[trix], Xml2[teix]
    _, Y_test = Yml2[trix], Yml2[teix]
     
    Y_pred = model.predict(X_test)
    
    mssave[0] += list(Y_pred[:,0])
    mssave[1] += list(Y_test)
     
    mse = mean_squared_error(Y_test, Y_pred)
    mse_scores.append(mse)
    
plt.scatter(mssave[0], mssave[1])
r_value, p_value = pearsonr(mssave[0], mssave[1])
print('r_value', r_value)
print('mean mse_scores', np.mean(mse_scores))
#%%
frequency_interval = {'theta': [4, 8], \
                      'alpha': [8, 13], \
                      'beta': [13, 30], \
                      'gamma': [30, 40]}

fis = list(frequency_interval.keys())

          
mssave2 = msFunction.msarray([len(fis)])  
alist = list(np.arange(-2, 2.1, 0.5))
for j in range(len(fis)):
    for alpha in np.arange(-2, 2.1, 0.5):
        ...
        for i in range(Xml2.shape[0]):
            ...
            s = frequency_interval[fis[j]][0] - 4
            e = frequency_interval[fis[j]][1] - 4
            
            X_test2[s:e] = X_test2[s:e] * alpha
             
            Y_pred = model.predict(np.array([X_test2]), verbose=0)
            ...
            mse = mean_squared_error(Y_test, Y_pred)
            mse_scores.append(mse)
            ypred_save.append(Y_pred[0][0])
            
        # plt.scatter(mssave[0], mssave[1])
        r_value, p_value = pearsonr(mssave[0], mssave[1])
        print()
        print('r_value', r_value)
        print(fis[j], 'mean mse_scores', np.mean(mse_scores))
        print('y value', np.mean(ypred_save))
        
        mssave2[j].append([np.mean(mse_scores), np.mean(ypred_save)])
        
#%%
for j in range(len(fis)):
    plt.plot(np.array(mssave2[j])[:,0], label=fis[j])
plt.legend()

#%%
for j in range(len(fis)):
    plt.plot(np.array(mssave2[j])[:,1], label=fis[j])
plt.legend()
...
X_test2 = np.array(X_test[0])
X_test2[13+4:30+4] = X_test2[13+4:30+4] + np.abs(np.mean(X_test2[13+4:30+4]))*2

X_test3 = np.array(X_test[0])
X_test3[13+4:30+4] = X_test3[13+4:30+4] - np.abs(np.mean(X_test3[13+4:30+4]))*2
