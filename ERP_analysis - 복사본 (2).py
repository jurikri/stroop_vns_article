...
def process_file(file_info, tix, eeg_df, SR=250):
    ...
    if xtmp.shape[0] > 420:
        ...
        #     xtmp4 = xtmp2 / np.mean(xtmp2[int(SR*0.25-0.1):int(SR*0.25+0.7)])
        #     xin.append(xtmp4)

        result_mssave = np.transpose(np.array(xtmp[:420]))
...
for ch in range(2):
    ...
    stds = np.std((Xch[:, :int(SR*(0.20+0.15))+1] - offset_standard), axis=1)
    stds_expanded = stds.reshape(-1, 1)

    Xch_std = Xch / stds_expanded
    
    offsets = np.nanmean(Xch_std[:,int(SR*(0.20)):int(SR*(0.20+0.15))], axis=1) - offset_standard
    offsets_expanded = offsets.reshape(-1, 1)
    
    Xnmr[:,:,ch] = (Xch_std - offsets_expanded)
...
if False:
    ...
    for cue in cues:
        ...
        # c0 = np.logical_and((np.array(Z_group[:,4], dtype=float) >1 ), \
        #                     (np.array(Z_group[:,4], dtype=float) <4 ))
        
        conditions = [c0, c1, c2, g_vns]
        # conditions = [c1, c2]
        vix = np.where(np.logical_and.reduce(conditions))[0]
        yvalues_vns = np.nanmedian(np.array(X_chmean)[vix], axis=0)
        ...
        vix = np.where(np.logical_and.reduce(conditions))[0]
        yvalues_sham = np.nanmedian(np.array(X_chmean)[vix], axis=0)
...
for i in range(len(mskey_list)):
    ...
    if not(np.isnan(acc)):
        mssave.append([msid, acc])
        
mssave = np.array(mssave)
rix = np.argsort(mssave[:,1])[:int(round(len(mssave)/2))]
low_group = mssave[rix, 0]
...
for cue in cues:
    ...
    for ans in ['맞음', '틀림']:
        ...
        yvalues_vnss, yvalues_shams = [], []
        c0 = (np.array(Z_group[:,4], dtype=float) > 3)
        ...
        #     if tmp[k] in low_group:
        #         vix_low.append(k)
        #     else:
        #         vix_high.append(k)
        # vix_low, vix_high = np.array(vix_low), np.array(vix_high)
        
        
        # yvalues_vns = np.median(np.array(X_chmean)[vix][vix_low], axis=0)
        # sem_vns = sem(np.array(X_chmean)[vix][vix_low], axis=0, nan_policy='omit')
        # yvalues_smoothing_vns = moving_average_smoothing(yvalues_vns, 20)
        
        # barplot[cue][ans][0] = np.array(X_chmean)[vix][vix_low]
        ...
        # yvalues_vns = np.median(np.array(X_chmean)[vix][vix_high], axis=0)
        # sem_vns = sem(np.array(X_chmean)[vix][vix_high], axis=0, nan_policy='omit')
        # yvalues_smoothing_vns = moving_average_smoothing(yvalues_vns, 20)
        
        # barplot[cue][ans][0] = np.array(X_chmean)[vix][vix_high]
...
for cue in cues:
    ...
    for ans in ['맞음', '틀림']:
        ...
        data_vns = np.mean(barplot[cue][ans][0][:, s:e], axis=1)
        data_sham = np.mean(barplot[cue][ans][1][:, s:e], axis=1)
...
for i in range(len(mskey_list)):
    for j in range(len(stimulus_types)):
        for t in range(len(session)):
            ...
            c1 = (Z_group[:,0] == stimulus)
            # c2 = (Z_group[:,4] == str(se))
            c2 = np.logical_or(Z_group[:,4] == str(session[t][0]), Z_group[:,4] == str(session[t][1]))
            ...
            if len(vix) > 0:
                ...
                X_semean.append(np.nanmedian(X_chmean[vix], axis=0))
                Z_semean.append([stimulus, '맞음', tscore, msid, t, '-', Z_group[vix][0][-1]])
